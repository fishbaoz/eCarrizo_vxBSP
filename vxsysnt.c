/* vxsysnt.c - VxWorks boot device format utility */
...
--------------------
01a,09may13,jjk  WIND00364942 - Adding Unified BSP.
*/
 ...
 * on odd-byte boundaries, boot sector offsets cannot be represented as a
 * standard C structure.  Instead, the following symbolic offsets are used to
 ...
 * #define VX_TO_DISK_16(src,pDst) (*(BYTE*)(pDst)=(BYTE)(src),
...
main(
    ...
    while (--argc > 0 )
        ...
        if (*p == '/' || *p == '-')
            ...
            if (!isalpha(*p) || !p[1] || p[1] != ':' || p[2])
                {
                /*
                 * relative or absolute path of mounted drive here not
                 * supported
                 */
     ...
     * bootfileflag: ReadBootSector (for disk data and check) write new boot
     * data to file
    ...
    if (bootfileflag && *DriveName=='\0')
        ...
        if (!saveflag && !bootfileflag && !systemdriveflag)
             ...
             * and writing a single sector only behaves same as exclusive
             * anyway even without lock, we lock it anyway to prevent
             * accidental damage of Windows System or Swap file drive.
        ...
        if (strchr( mediatype_harddisk, Geometry.MediaType) != NULL)
            ...
            if (!DeviceIoControl(
                ...
                {
                printf("%s Failed: Get Partition Info Removable or Fixed Hard \
                        Disk %s\n",fname, DriveName);
                return cleanup( 1, &Cleanup );
                }
                
            /*
             * We allow here any FATx partition type, since these partition
             * sector flags are not always accurately matching the actually
             * formatted volume inside of the partition. We check in detail
             * later. For vxsys booting we allow only FAT12 and FAT16
             * (vxsys also supports hardisks without MBR and boot sector
             * directly at the beginning of the drive, and also vxlongname
             * FAT variant. Because we are running on Windows here, and those
             * do not making sense for exchanging disks between VxWorks and
             * Windows, we ignore them here)
             */

            else if ( strchr( parttype, PartInfo.PartitionType) == NULL )
                {
                printf("%s Failed: Unknown Partition Type %d on Removable or \
                    Fixed Hard Disk %s\n",
                    fname,
            ...
            SectorsPerDisk.QuadPart =
                    PartInfo.PartitionLength.QuadPart / BytesPerSector;
            hiddensectors =
                    (DWORD)(PartInfo.StartingOffset.QuadPart / BytesPerSector);
    ...
    /*
     * Although experimentally not reproducable on WinXP and Win2000, some
     * literature claims that the buffer needs to be sector aligned in memory
     * for being suitable for direct disc access.
    ...
    if (bootfileflag && *DriveName=='\0')
        {
        /*
         * Instead of reading from the disk we create an empty template boot
         * sector
         */
        ...
        newfilepos.u.LowPart =
            SetFilePointer( hDrive, 0, &(newfilepos.u.HighPart), FILE_BEGIN );
        ...
        printf("%s Progress: Original Bootloader and Disk Parameters read \
                from %s\n",fname, DriveName);
        ...
        if (!ignoreflag)
            ...
            RootDirSectors = ((DISK_TO_VX_16(pBoot+DOS_BOOT_MAX_ROOT_ENTS) *
            ...
            /*
             * Rounded down; Not cluster index count, since first cluster has
             * index2. Zero is not allowed because of division by zero.
             * We will check this later, therefore so far we assign a pseudo
             * value to CountofClusters in this case.
             */
            ...
            if (DISK_TO_VX_8(pBoot+DOS_BOOT_SEC_PER_CLUST) == 0)
                ...
                {
                CountofClusters =
                    DataSec / DISK_TO_VX_8(pBoot+DOS_BOOT_SEC_PER_CLUST);
                }

            /*
             * Now checking confidence and consistency whether this is really
             * a boot parameter block in the boot sector, or looks only
             * partially as such
             *
             * Not checked since Filesystem works without this value:
             * - 0xF8 is standard for fixed disk, 0xF0 for floppy disk, other
             *      0xF9..0xFF also valid.
             ...
             * - DOS_BOOT_SIG_REC to indicate that serial, label, and
             *      typestring are present
             * - Unused space:  Check if
             *   RootDirSectors*BytesPerSector >
             *       DISK_TO_VX_16(pBoot+DOS_BOOT_MAX_ROOT_ENTS) * 32 ||
             * - in FAT table:
             *   Strictly, first Byte of FAT need to be identical to this,
             *      but we don't check this.
             *   For FAT entry 1 and FAT16 or FAT32: Highest Bit=0 shutdown
             *      error, SecondHighest Bit=0
             *   DiskError on last run;
             *   Other Bits of FAT entry 0 and 1 always 1.
             *   Special FAT entrys: -9 for Bad Block, -1..-8 for
             *      End of file (only -1 used).
             *   FAT32 has 28Bits. Therefore Bit28..31 always zero, even for
             *      negative values as above.
             */

            if ( /*
                  * Boot Sector Signature is expected at offset 510 of Boot
                  * Sector. Some Software is known to place it into the last
                  * two Bytes of the sector which might be different.
                  * We are tolerant.
                  */
                  ...
                  * This is the only valid way to decide between FAT12/16/32
                  * types!!! Here we allow only FAT12 and FAT16
                  */

                 CountofClusters >= DOS_FAT_16BIT_MAX ||
                    DISK_TO_VX_16(pBoot+DOS_BOOT_BYTES_PER_SEC) !=
                    BytesPerSector ||
                 ...
                 /*
                  * Although more as 2 FAT copies are legal, this is so
                  * weird that we do not accept this
                  */
                 ...
                 /*
                  * If 0 we cannot compute the count of clusters and we have
                  * a division by zero needs to be a power of 2 between 1 and
                  * 128 Cluster size max 32KB!
                  */

                 DISK_TO_VX_8(pBoot+DOS_BOOT_SEC_PER_CLUST) *
                    BytesPerSector > 0x8000 ||
                    strchr("\x01\x02\x04\x08\x10\x20\x40\x80",
                    DISK_TO_VX_8(pBoot+DOS_BOOT_SEC_PER_CLUST)) == NULL ||

                 /*
                  * If DataSec >= TotSec we had an overflow on subtracting
                  * special sectors from TotSec
                  */
                 ...
                 /*
                  * FatType is Bits per FAT entry. Since we do not treat FAT32
                  * here we can ignore integer overflows FAT need to fit in
                  * FAT table
                  */

                 ((CountofClusters+2) *
                    (CountofClusters < 4085 ? 3 : 4) +
                    2 * BytesPerSector - 1) / (2 * BytesPerSector) > FatSz )
                {
                printf("%s Failed: No or illegal FAT12 or FAT16 Boot Sector \
                        for booting found on %s\n",fname, DriveName);
                return cleanup( 1, &Cleanup );
                }
            else
                {
    ...
    if (restoreflag )
        ...
        /*
         * We either read the file or write it, therefore we don't need to
         * close the handle for making it free for a later write
         */

        printf("%s Progress: Bootloader and Disk Params read from File %s\n",
                fname, sectorfilename);
        }
    else if (!saveflag)
        {
         ...
         * For non bootable data partitions this is not relevant, because they
         * are loaded by an external OS. For the bootcode to find itself hidden
         * sectors must be equal to LBA of bootsector.
         * For extended partitions, Microsoft formatters are using the distance
         * to the preceding Ext Part Sectors as hidden sectors, which is
         * usually 0x3f and which prevents that extended partitions are bootable
         */

        VX_TO_DISK_32( hiddensectors, pBoot+DOS_BOOT_NHIDDEN_SECS);

        /*
         * Maybe replace the OEM signature string at Byte 3 of Boot sector
         * with length 8 here.
