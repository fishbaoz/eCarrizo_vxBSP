...
/*
 * Copyright 2012-2013 (c) Wind River Systems, Inc.
 *
...
01a,10jul12,j_z  create based on 01b version of itl_nehalem.
*/
...
#define MMAR_DESC_TYPE_ACPI_NVS     4 /* requires save between NVS sessions */
...
void multibootInit ()
    ...
    if (pMbInfo->flags & 4)
        ...
        char * str1 = BOOTLINE_EXTERN_MARKER;
        int i,j,k;
...
#else
        char * bootline;
        bootline = strstr((char*)pMbInfo->cmdline, BOOTLINE_EXTERN_MARKER);
#endif
        if (NULL != bootline)
            {
            strncpy (sysBootLine,
                     bootline + strlen(BOOTLINE_EXTERN_MARKER), BOOT_LINE_SIZE);
            strncpy ((char *)BOOTLINE_EXTERN_ADDR, sysBootLine, BOOT_LINE_SIZE);
            }
        }
    }
...
void multibootShow ()
    ...
    if (flags & 1)
        printf ("mem_lower: 0x%x mem_upper: 0x%x\n",
                (unsigned int)pMbInfo->mem_lower,
                (unsigned int)pMbInfo->mem_upper);
    ...
    if (flags & 0x40)
        ...
        printf ("mmap_areas type     start address     "
                "          region length\n");
