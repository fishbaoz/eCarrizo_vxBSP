...
01b,06jul12,j_z  add some multiboot macros.
01a,25mar11,lll  created based on itl_nehalem 01a version.
*/
...
/*
 * The following defines the address of an external bootline reserved area
 * that can be provided to VxWorks (512 bytes is reserved for this buffer).
 * This address can be used with any type of suitable loader including GRUB1,
 * GRUB2, Simics, etc., to provide a VxWorks bootline. A bootline prefix marker
 * string (i.e., a signature) is defined to uniquely identify the start of
 * valid VxWorks bootline params.
 */

/*
 * NOTE: Be careful with these addresses; they shouldn't conflict with other
 * definitions in configAll.h
 */

#define BOOTLINE_EXTERN_ADDR        0x106000
#define BOOTLINE_EXTERN_MAXLEN      BOOT_LINE_SIZE /* see configAll.h */
#define BOOTLINE_EXTERN_MARKER      "bootline:"

#define MULTIBOOT_BOOT_FLAG_ADRS (BOOTLINE_EXTERN_ADDR + BOOTLINE_EXTERN_MAXLEN)
#define MULTIBOOT_SCRATCH                 0x106210
#define MULTIBOOT_BOOT_LOAD_ADRS          0x106220
#define MULTIBOOT_BOOTROM_SAVED_FLAG_ADRS 0x106228 /* was bootrom saved ? */
#define MULTIBOOT_BOOTROM_SIZE            0x106230
#define MULTIBOOT_BOOTROM_ADRS            0x106238
...
/*
 * Absolute offset in romInit.s. If romInit.s has been changed, must be
 * synced up with it
 */

#define ROM_WARM_HIGH               0x20    /* warm start entry p */
#define ROM_WARM_LOW                0x30    /* warm start entry p */
#define ROM_COLD_START              0x40    /* cold start entry p */
#define MULTI_STAGE_RAM_START       0xc0    /* multi-stage boot ram entry */
...
#define PIC_EARLY_EOI_IRQ0          2      /* lower IRQ0 ISR to lower lvl */
...
#define PCI_INT_VECTOR_TO_IRQ(vector) (sysPciIvecToIrq((_Vx_usr_arg_t)vector))
 ...
 * willing to customize this BSP by finding unused IRQs. For example,
 * IRQ5 and IRQ2(9) are often unused.  N_UART_CHANNELS must be changed to
 * reflect the actual number of serial ports supported. Rather than changing
 * UART base definitions here, you can override them in config.h for
 * a specific board configuration since only config.h includes pc.h directly.
 * See the I8250_CHAN_PARAS devParas structure in sysSerial.c for additional
...
#define N_UART_CHANNELS         0    /* bios default setting on eKabini (pci bus 0 dev 0x14 func 3 reg 0x44) */
